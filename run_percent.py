...
df = df[(df['percent']>0.002)]
df.to_csv("unique_interactions.csv", index=False, header=True)
...
for t,c in zip(df.type,range(len(df.type))):
    if t == 'HB':
        y1.append(df.percent[c])
    else:
        y1.append(0)

outfile = open('h_bond.csv','w')
out = csv.writer(outfile)
out.writerows(zip({'res'},{'percent'}))
...
with open('h_bond.csv') as fin:
    lines = fin.readlines()
lines[0] = lines[0].replace('res,percent', 'num,res,percent')
...
with open('out.csv') as fin:
    lines = fin.readlines()
lines[0] = lines[0].replace('num-res,percent', 'res,percent')
...
ylabel(r'Interaction Fraction', fontsize=10,weight='bold')
...
plt.legend(fancybox=True, framealpha=0, prop={'size': 6},loc=0)
